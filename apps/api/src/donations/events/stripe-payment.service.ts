...
export class StripePaymentService {
  ...
  async handleSubscriptionCreated(event: Stripe.Event) {
    ...
    rdDto.amount = subscription.items.data[0].price.unit_amount as number
...
*/
  ...
  async checkForCompletedCampaign(campaignId: string) {
    ...
    if (updatedCampaign.state === CampaignState.complete) {
      const recurring = await this.recurringDonationService.findAllRecurringDonationsByCampaignId(campaignId)
