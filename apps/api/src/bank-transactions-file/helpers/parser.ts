...
export function parseBankTransactionsFile(fileBuffer):{ payment: CreateBankPaymentDto; paymentRef: string }[]{
    ...
    for (const item in items) {
      for (const movement in items[item].AccountMovement) {
        if (items[item].AccountMovement[movement].MovementType[0] === 'Credit') {
          ...
          payment.extCustomerId =
            items[item].AccountMovement[movement].Account[0].BankClientID[0]
          payment.extPaymentIntentId =
            items[item].AccountMovement[movement].DocumentReference[0]
