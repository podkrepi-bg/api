...
import { UpdateCampaignDto } from '../domain/generated/campaign/dto'
...
export class CampaignService {
  ...
  //Cant find inputDto.id
  // async editCampaign(inputDto: UpdateCampaignDto): Promise<Campaign> {
  //   const [campaign, index] = this.findCampaign(inputDto.id);
  //   const updatedCampaign = {...campaign }
  // if(inputDto.title) {
  //   updatedCampaign.title = title;
  // }
  //   this.campaign[index] = updatedCampaign;
  // }

  async deleteCamp(campId: string) {
    const index = this.findCampaign(campId)[1]
    const campaigns = await this.prisma.campaign.findMany({
      include: {
        vaults: {
          select: {
  ...
  private async findCampaign(id: string) {
    ...
    const campaignIndex = campaigns.findIndex((camp) => camp.id === id)
